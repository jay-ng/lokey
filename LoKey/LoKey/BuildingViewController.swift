...
class BuildingViewController: UIViewController {
    ...
    func drawBuilding(floors: Int, activeFloor: Int, carPosition: String){
        ...
        //-------------------------------------------- upper half of screenView  "floor overview"
            ...
            let FPbase : Double = (2.0 * screen.height) - (style.marginBottom + style.marginTop)
            ...
            activeFloorplan.addLine(to: CGPoint(x: (style.marginLeft + style.inset), y: screen.height + style.marginTop))
            activeFloorplan.addLine(to: CGPoint(x: screen.width - (style.marginLeft + style.inset), y: screen.height + style.marginTop))
            ...
            // Quadrants Divider
            let floorPlanDividerLayer = CAShapeLayer()
            let floorPlanDivider = UIBezierPath()
        
            floorPlanDivider.move(to: CGPoint(x: (screen.width / 2), y: screen.height + style.marginTop ))
            floorPlanDivider.addLine(to: CGPoint(x: screen.width/2, y: FPbase))
            floorPlanDivider.move(to: CGPoint(x:(style.marginLeft + 0.5 * style.inset), y: (1.5 * screen.height) - style.marginTop))
            floorPlanDivider.addLine(to: CGPoint(x: (screen.width - style.marginRight) - (0.5 * style.inset), y: (1.5 * screen.height) - style.marginTop))
            floorPlanDivider.close()
        
            floorPlanDividerLayer.path = floorPlanDivider.cgPath
            floorPlanDividerLayer.lineDashPattern = [1, 8]
            floorPlanDividerLayer.lineWidth = 2.0
            floorPlanDividerLayer.lineCap = kCALineCapRound
            floorPlanDividerLayer.strokeColor = UIColor.white.cgColor
            floorPlanDividerLayer.zPosition = 0.6
        
            screenView.layer.addSublayer(floorPlanDividerLayer)
            //
        
            // Center Quadrant
            let floorCenterPath = UIBezierPath(arcCenter: CGPoint(x: screen.width/2,y:1.5*screen.height - style.marginTop), radius: CGFloat(screen.width/6), startAngle: CGFloat(0), endAngle: CGFloat(Double.pi * 2), clockwise: true)
            let floorCenterLayer = CAShapeLayer()
            floorCenterLayer.path = floorCenterPath.cgPath
            floorCenterLayer.lineDashPattern = [1, 3]
            floorCenterLayer.lineWidth = 2.0
            floorCenterLayer.lineCap = kCALineCapRound
            floorCenterLayer.strokeColor = UIColor.white.cgColor
            floorCenterLayer.fillColor = style.floorColor.cgColor
            floorCenterLayer.zPosition = 0.7
        
            screenView.layer.addSublayer(floorCenterLayer)
            ...
            var carMark = UIBezierPath()
        
            if carPosition != "N/A" {
                switch(carPos){
                    
                case "ne":
                    carMark.move(to: CGPoint(x: (screen.width / 2), y: (1.5 * screen.height) - style.marginTop))
                    carMark.addLine(to: CGPoint(x: (screen.width / 2), y: screen.height + style.marginTop))
                    carMark.addLine(to: CGPoint(x: screen.width - (style.marginLeft + style.inset), y: screen.height + style.marginTop))
                    carMark.addLine(to: CGPoint(x: screen.width - (style.marginLeft + 0.5 * style.inset), y: (1.5 * screen.height) - style.marginTop))
                    
                    break;
                case "nw":
                    carMark.move(to: CGPoint(x:(style.marginLeft + 0.5 * style.inset), y: (1.5 * screen.height) - style.marginTop))
                    carMark.addLine(to: CGPoint(x: style.marginLeft + style.inset, y: screen.height + style.marginTop))
                    carMark.addLine(to: CGPoint(x: (screen.width / 2), y: screen.height + style.marginTop))
                    carMark.addLine(to: CGPoint(x: (screen.width / 2), y: (1.5 * screen.height) - style.marginTop))
                    break;
                case "se":
                    carMark.move(to: CGPoint(x:(screen.width / 2), y: FPbase))
                    carMark.addLine(to: CGPoint(x: (screen.width / 2), y: (1.5 * screen.height) - style.marginTop))
                    carMark.addLine(to: CGPoint(x: screen.width - (style.marginLeft + 0.5 * style.inset), y: (1.5 * screen.height) - style.marginTop))
                ...
                case "sw":
                    ...
                    carMark.addLine(to: CGPoint(x:(style.marginLeft + 0.5 * style.inset), y: (1.5 * screen.height) - style.marginTop))
                    carMark.addLine(to: CGPoint(x: (screen.width / 2), y: (1.5 * screen.height) - style.marginTop))
                    carMark.addLine(to: CGPoint(x:(screen.width / 2), y: FPbase))
                    break;
                case "c":
                    carMark = UIBezierPath(arcCenter: CGPoint(x: screen.width/2,y:1.5*screen.height - style.marginTop), radius: CGFloat(screen.width/6), startAngle: CGFloat(0), endAngle: CGFloat(Double.pi * 2), clockwise: true)
                default:
                    //Center
                    break;
                }
                ...
                carMarkLayer.fillColor = Utils.primaryColor.withAlphaComponent(0.6).cgColor
                if carPos != "c" {
                    carMarkLayer.zPosition = 0.5
                } else {
                    carMarkLayer.zPosition = 0.8
                    //carMarkLayer.lineDashPattern = [1, 3]
                    //carMarkLayer.lineWidth = 2.0
                    //carMarkLayer.strokeColor = UIColor.white.cgColor
                }
    ...
    override func viewDidAppear(_ animated: Bool) {
        if location != nil {
            if userDevice != nil {
                if let carCurrentPosition = userDevice.currentLocation {
                    if location.id == carCurrentPosition {
                        self.currentActiveFloor = instance.getRelativeFloor(location: self.location, altitude: userDevice.currentAltitude)
                    }
                }
            }
            if location.building.floors.count > self.currentActiveFloor {
                self.drawBuilding(floors: location.building.floors.count, activeFloor: self.currentActiveFloor, carPosition: self.currentCardinal)
            } else {
                self.drawBuilding(floors: location.building.floors.count, activeFloor: self.location.building.floors.count-1, carPosition: self.currentCardinal)
            ...
            if (userDevice != nil) {
                if let carCurrentPosition = userDevice.currentLocation {
                    if location.id == carCurrentPosition {
                        ...
                        self.currentCardinal = self.instance.getCardinalPosition()
                    }
                }
            }
    ...
    func demoLoop(){
        ...
        let f = 4;
        let a = 0;
        Timer.scheduledTimer(timeInterval: 0, target: self, selector: #selector(self.demo), userInfo: ["floors": f, "activeFloor": a, "carPosition": "c"], repeats: false)
