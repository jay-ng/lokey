...
class Instance : NSObject, CLLocationManagerDelegate {
    ...
    private var stopActivityTracking: Bool = false
    ...
    init(_ platform: Platform){
        ...
        self.stopActivityTracking = false
    ...
    public func run(){
        ...
        self.locationManager.pausesLocationUpdatesAutomatically = false // ? override
        if (CLLocationManager.authorizationStatus() != CLAuthorizationStatus.authorizedAlways) {
            self.locationManager.requestAlwaysAuthorization()
        }
        // TODO: Check that user has given loction. Mandatory
        self.locationManager.desiredAccuracy = kCLLocationAccuracyBestForNavigation
        self.locationManager.distanceFilter = kCLDistanceFilterNone // Only get location update after 2m difference
    ...
    func startAssesmentLoop(){
        ...
        if(!self.isvalid){
            ...
            self.log(">> Starting Assesment Loop", LOG_LEVEL.log)
            self.assesmentLoop()
    ...
    @objc private func assesmentLoop(){
        ...
        for connection in AVAudioSession.sharedInstance().currentRoute.outputs{
            ...
            if let activeIndex : Int = self.state.connectionIdentifiers.index(of: connection.uid){
                ...
                log("- New Audio Connection Detected: \(connection.portName)", LOG_LEVEL.log)
                    ...
                    if let unwrappedInterface : NSDictionary = CNCopyCurrentNetworkInfo(interface as CFString){
                        if let connection = unwrappedInterface as? [String : AnyObject] {
                            if let activeIndex : Int = self.state.connectionIdentifiers.index(of: connection["SSID"] as! String){
                                ...
                                self.state.addConnection(newConnection)
                                self.state.activeConnections.append(newConnection.id)
                                log("- New WiFi Connection Detected: \(newConnection.name)", LOG_LEVEL.log)
                            }
                        }
                    }
                }
                ...
                if let deviceConnectionId : Int = userDevice.connection{
                    if self.state.activeConnections.contains(deviceConnectionId){
                        ...
                        if(!self.state.carConnected){
                            // Car has connected
                            log("!> Car has recently connected", LOG_LEVEL.log)
                            ...
                            self.deleteAllCarPositionsInLog()
                            carUpdate = true
                        } else {
                            // Car still connected
                        ...
                        for region in (regions) {
                            if region.identifier == "carRegion" {
                                self.log("Removing Car Region", LOG_LEVEL.log)
                        ...
                        if(self.state.carConnected){
                            ...
                            log("!> Car has recently disconnected", LOG_LEVEL.log)
                            isSignificant = true
                            ...
                            self.addCarPositionToLog(coordinate: car.currentPosition!)
                            self.addCarPositionToLog(coordinate: car.currentPosition!)
                            
                            if self.samplingExitCoords.count > 2 {
                            ...
                            if self.carCurrentFloor == 0 {
                                self.log("-- Car set on floor: Ground Floor", LOG_LEVEL.log)
                            } else {
                                self.log("-- Car set on floor: \(self.carCurrentFloor)", LOG_LEVEL.log)
                            ...
                            if let currentPosition : CLLocationCoordinate2D = self.locationManager.location?.coordinate {
                                ...
                                if let currentLoc : Location = self.state.getCurrentLocation() {
                                    ...
                                    self.log("Monitoring: Car Region", LOG_LEVEL.log)
                                    let carRegion = CLCircularRegion(center: currentPosition, radius: 30.0, identifier: "101010")
                                    self.monitoredRegions.append(carRegion)
                                    self.locationManager.startMonitoring(for: carRegion)
                                ...
                                if self.quadInformationPresented == false {
                                    ...
                                    let averagedParkedCoordinate: Coordinate = self.state.getCoordinateCenter(coordinates: self.locationCarPositionLog)
                                    
                                    if(car.currentLocation != nil){
                                        ...
                                        log("quadrant information presented: \(self.quadInformationPresented)", LOG_LEVEL.log)
        ...
        if self.assesmentCount % 5 == 0 || isSignificant {
            ...
            if self.assesmentCount % 10 == 0 || isSignificant {
    ...
    func trackLocation(_ location: Location){
        ...
        //if (!monitoredRegions.contains(region)) {
            if (CLLocationManager.isMonitoringAvailable(for: CLCircularRegion.self)) {
                ...
                log("- Area monitoring for location: \(location.name)", LOG_LEVEL.log)
            }
        //}
    }
    ...
    func locationManager(_ manager: CLLocationManager, didEnterRegion region: CLRegion) {
        ...
        log("--- User entered an area with id \(region.identifier) ---", LOG_LEVEL.log)
    ...
    func locationManager(_ manager: CLLocationManager, didExitRegion region: CLRegion) {
        ...
        log("--- User exited an area with id \(region.identifier) ---", LOG_LEVEL.log)
        ...
        if let loc = state.getCurrentLocation() {
            ...
            if samplingExitCoords.count > 9 {
                ...
                if confident < 2 {
                    ...
                    //self.stopAltimeter()
                    self.stopActivityTracking = true
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let newLocation = locations.last {
            //self.log("-- Horizontal Accuracy: \(newLocation.horizontalAccuracy)", LOG_LEVEL.log) // disable debugging
            ...
            for region in self.monitoredRegions{
                if let trackedLocation : Location = self.state.getLocation(id: Int(region.identifier)!){
                    ...
                    if(d < self.state.getRadius(center: trackedLocation.center, foundations: trackedLocation.building.foundation)){
                        self.log("Current Location Identified by Position Evaluation -> \(trackedLocation.name)");
                        self.events.append(
                            Event(
                                timestamp: Fmt.getTimestamp(),
                                type: EVENT_TYPE.significant,
    ...
    func trackUserActivity(){
        altimeterManager.startRelativeAltitudeUpdates(to: OperationQueue.main) { (altimeterData: CMAltitudeData?, error: Error?) in
            //self.log("reading current altitude data"); // disable for debugging
            self.currentAltitude = (altimeterData?.relativeAltitude.doubleValue)!;
            
            if(self.stopActivityTracking == true){
                self.log("Altitude data collection stopped", LOG_LEVEL.log)
        ...
        self.activityManager.startActivityUpdates(to: OperationQueue.main) { (activity: CMMotionActivity?) in
            if (activity?.automotive)!{
                //self.log("Motion Type: Driving", LOG_LEVEL.log);
                self.userActivity = USER_ACTIVITY.driving
            } else if (activity?.walking)!{
                //self.log("Motion Type: Walking", LOG_LEVEL.log);
                self.userActivity = USER_ACTIVITY.walking
                
            } else if (activity?.stationary)!{
                //self.log("Motion Type: Stationary", LOG_LEVEL.log);
                self.userActivity = USER_ACTIVITY.stationary
            }
            
            if(self.stopActivityTracking == true){
                //self.log("Activity Updates have Stopped", LOG_LEVEL.log)
    ...
    func getQuadInfoForLocation(averageCoordinate: Coordinate, carLocation: Location){
        ...
        var centerLocation: Bool = false
        var bearing: Double = 0.0
        self.quadInformationPresented = true
        var carCurrentLocation: Location = carLocation
        
        centerLocation = self.centerCheck(carCoordinate: averageCoordinate, centerCoord: carLocation.center, carLocation: carLocation)
                ...
                if carCurrentLocation.building.foundation.count > 0 {
                    ...
                    self.headerString = self.getHeaderString(bearing_: bearing, locationInCenter: centerLocation)
    ...
    func centerCheck(carCoordinate: Coordinate, centerCoord: Coordinate, carLocation: Location)-> Bool{
        var distanceSum: Double = 0.0
        var carDistanceFromCenter = 0.0
        var centerThreshold: Double = 0.0
        for i in 0...carLocation.building.foundation.count - 1{
            distanceSum += self.state.getDistance(c1: centerCoord, c2: carLocation.building.foundation[i])
            
        }
        centerThreshold = 0.25 * (distanceSum/Double(carLocation.building.foundation.count))
        //log("center threshold: \(centerThreshold)", LOG_LEVEL.debug)
        carDistanceFromCenter = self.state.getDistance(c1: carCoordinate, c2: centerCoord)
        //log("car center from distance: \(carDistanceFromCenter)", LOG_LEVEL.log)
        if carDistanceFromCenter <= centerThreshold{
            //log("car is within center Threshold", LOG_LEVEL.log)
            return true
        }else{
            //log("car is not within center Threshold", LOG_LEVEL.log)
            return false
        }
    }
    
    func getHeaderString(bearing_: Double, locationInCenter: Bool) -> String{
        var header: String = "N/A"
        
        if locationInCenter == false {
            ...
            header = "c"
            log("- Car location in center area of parking deck", LOG_LEVEL.log)
        }
    ...
    func addCarPositionToLog(coordinate: Coordinate){
        let coordinates_ = coordinate
    ...
    func loadState(_ packedState : [String:AnyObject]){
       ...
       // if let packedState : [String:AnyObject]  = self.localStorage.dictionary(forKey: "loKey") as? [String : AnyObject]{
            ...
            log("-- User unpacked in \(CFAbsoluteTimeGetCurrent() - time_startUserLoad) s") //-- stat
            ...
            if let packedLocations : [String:AnyObject] = packedState["locations"] as? [String:AnyObject]{
                for (id,packedLocation) in packedLocations{
                    ...
                    log("- Location ID Unpacked: \(id)")
                }
            }
            log("-- Locations unpacked in \(CFAbsoluteTimeGetCurrent() - time_startLocLoad) s") //-- stat
            ...
            if let packedConnections : [String:AnyObject] = packedState["connections"] as? [String:AnyObject]{
                for (id,packedConnection) in packedConnections{
                    ...
                    log("- Connections ID unpacked: \(id)")
                }
            }
            log("-- Connections unpacked in \(CFAbsoluteTimeGetCurrent() - time_startConnLoad) s") //-- stat
            ...
            if let packedDevices : [String:AnyObject] = packedState["devices"] as? [String:AnyObject]{
                for(id, packedDevice) in packedDevices{
                    ...
                    log("- Device ID unpacked: \(id)")
                }
            }
            log("-- Deviced loaded in \(CFAbsoluteTimeGetCurrent() - time_startDeviceLoad) s\n") //-- stat
        
        
            log("-- State loaded in \(CFAbsoluteTimeGetCurrent() - time_startStateLoad) s\n") //-- stat
        //} else {
